...
export function Welcome({ ctx }) {
  ...
  return html`<div className="welcome">
    ...
    <h1>내 포트폴리오</h1>
    <p className="hint">증권사 앱의 잔고 화면을 옆에 띄워 두고, 보이는 숫자를 그대로 옮겨 적으면 됩니다. 계좌 하나에 5분쯤 걸려요.</p>
    <ol className="hint" style=${{ margin: 0, paddingLeft: '20px', display: 'grid', gap: '4px' }}>
      <li>계좌 만들기 (증권사·수수료)</li><li>보유 종목 넣기 (수량·평균단가)</li><li>현금 넣기 (예수금)</li><li>구글 자동 시세 연결 또는 현재가 입력</li>
    </ol>
    ...
    ${resume ? html`<button className="link2" type="button" onClick=${async () => { await saveSettings({ onboarded: true }); go('#/'); }}>나중에 하고 홈으로</button>` : null}
    <p className="sm">데이터는 이 폰에만 저장됩니다. 구글 연결 시 내 구글 계정에 자동 백업돼요.</p>
  </div>`;
}
...
export function StartHoldings({ ctx, params }) {
  ...
  if (!acc) return html`<${TopBar} title="종목 넣기" back="#/start" /><p className="hint" style=${{ padding: '16px' }}>계좌를 찾을 수 없어요.</p>`;
...
export function StartPrices({ ctx }) {
  ...
  useEffect(() => {
    ...
    refreshQuotes().catch((e) => toast(explain(e))).finally(() => setDone(true));
  }, []);
  const finish = async () => { await saveSettings({ onboarded: true }); go('#/'); };
  return html`<${TopBar} title="현재가 · 환율" back="#/start" step="4/4" />
    ${!done ? html`<p className="hint" style=${{ padding: '16px' }}>구글에서 시세를 받는 중이에요…</p>`
